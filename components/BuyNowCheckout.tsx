...
import { ArrowLeft,CheckCircle2,Copy,Loader2,MapPin,MessageCircle,RefreshCw,ShieldCheck,Truck,Upload } from "lucide-react";
  ...
  return <main className="min-h-screen bg-base px-4 pb-16 pt-28 text-emerald sm:px-6"><div className="mx-auto max-w-4xl">
    ...
    {step === 1 && <section className="mt-5 overflow-hidden rounded-[30px] border border-emerald/10 bg-white/75 shadow-xl">
      ...
      <div className="p-5 sm:p-7">
        ...
        <div className="mt-6 rounded-[26px] border border-emerald/10 bg-gradient-to-br from-white to-base p-4 shadow-sm sm:p-5"><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-gold/10 text-gold"><MapPin size={17}/></span><div><h2 className="font-heading text-xl">Delivery information</h2><p className="mt-0.5 text-[11px] leading-5 text-emerald/45">Enter your pincode first. City and state are filled automatically when postal data is available.</p></div></div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><input value={form.email} onChange={e => updateField("email", e.target.value)} placeholder="Email address" type="email" autoComplete="email" className="field"/><p className="mt-1.5 px-1 text-[10px] leading-4 text-emerald/40">Add your email to receive confirmation and tracking updates.</p></div>
    ...
    {step === 2 && <section className="mt-5 rounded-[30px] border border-emerald/10 bg-white/75 p-5 shadow-xl sm:p-7"><button type="button" onClick={()=>{setError("");setStep(1);}} className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald/10 bg-base px-3 py-2 text-[11px] font-semibold text-emerald/65"><ArrowLeft size={14}/>Back to delivery</button><p className="text-[10px] font-semibold uppercase tracking-[.25em] text-gold">Step 2</p><h2 className="mt-2 font-heading text-3xl">Make payment</h2><p className="mt-2 text-sm text-emerald/55">Pay exactly <b>{money(Number(order?.amount || payableTotal))}</b>. Your delivery charge is already included.</p>
